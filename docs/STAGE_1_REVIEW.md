...
1. 产品化 API 契约与输入约束。
2. Prompt 版本化与输出 schema 固化。
3. 样例需求数据集。
4. 最小评测脚本。
5. 结果展示与演示入口。
6. 第 2 阶段验收总结。

当前第 2.1 到第 2.3 步已完成：API 支持 `requirement` + 可选 `context`，Prompt 固定为 `requirement-analysis-v1`，样例数据集位于 `evals/requirement_samples.jsonl`。

下一步建议进入第 2.4 步：实现最小评测脚本，验证输出结构和关键字段质量。
