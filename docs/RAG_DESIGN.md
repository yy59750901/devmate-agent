...
```text
FastAPI + LlamaIndex + OpenAI-compatible LLM/Embedding + 本地持久化索引
```

职责：

- FastAPI：在 `agent-python` 中暴露 RAG HTTP API。
- LlamaIndex：负责文档加载、切分、索引、检索、query engine、回答综合和 sources。
- OpenAI-compatible embedding：负责把文档片段和问题转成向量，用于语义检索。
- OpenAI-compatible chat LLM：负责基于检索片段生成自然语言回答。
...
```text
LlamaIndex + OpenAI-compatible LLM/Embedding + 本地持久化索引
```
...
第一版 RAG 允许 `agent-python` 直接调用 OpenAI-compatible LLM 和 embedding 服务，便于快速跑通真实业务形态的完整 RAG 问答。

配置文件：

```text
agent-python/config/config.example.yaml
agent-python/config/config.local.yaml
```
...
后续可再演进为 Python 通过 Go Backend LLM Gateway 统一调用模型。
...
1. 安装并接入 LlamaIndex、OpenAI-compatible LLM 和 embedding。
2. 加载 `docs/*.md` 和 `README.md`。
3. 构建或加载本地索引。
4. 暴露 `POST /api/rag/query`。
5. 使用 LLM 基于检索片段生成 `answer`。
